...
xwalk <- data.frame(stname = state.name, state = state.abb)
pop <- pop %>%
  left_join(xwalk) %>%
...
mpv <- mpv %>%
  mutate(
    race = ifelse(race == "Unknown race", NA, race),
...
mpv_imps <- mpv %>%
  ...
  mutate(race = factor(race), gender = factor(gender), year = factor(year))

# impute ------------------------------------------------------------------
imps_temp <- mice(mpv_imps, m = 20, maxit = 20)

saveRDS(imps_temp, "./data/imps.RDS")

mpv_imputed <- complete(imps_temp, action = "long")

### output
write_csv(
  mpv_imputed %>%
    select(.imp:state),
  "./data/mpv_imputed.csv"
)
...
t %>%
  group_by(.imp) %>%
  summarize(age_mn = mean(age, na.rm = T), age_sd = sd(age, na.rm = T))
