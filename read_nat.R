...
mpv_yr <- mpv_yr |>
  ...
  tidyr::complete(.imp, year, age, gender, race_ethn, fill = list(n = 0))
...
m1_1 <- read_tsv("./data/wonder_mort_race_age_sex_00_20.txt") |>
  ...
  mutate(deaths = as.numeric(deaths), population = as.numeric(population)) |> # coerced suppressed to NA, small cells
...
m3 <- read_tsv("./data/wonder_provisional_mort.txt") |>
  ...
  rename_all(tolower) |>
  select(age, gender, year, deaths, race_ethn, population) |>
  mutate(deaths = as.numeric(deaths), population = as.numeric(population)) |> # coerced suppressed to NA
  group_by(age, gender, year, race_ethn) |>
  summarize(
    deaths = sum(deaths, na.rm = T),
    population = sum(population, na.rm = T)
